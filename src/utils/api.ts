...
const getFilterString = (filters: Filters | undefined) => {
  ...
  if (filters?.text && filters.text.length > 0)
    filterStr += `&filter[text]=${filters.text}`;
  ...
  if (filters?.season && filters.season.length > 0)
    filterStr += `&filter[season]=${filters.season}`;
  ...
  if (filters?.ageRating && filters.ageRating.length > 0)
    filterStr += `&filter[ageRating]=${filters.ageRating}`;
...
export const animeUrl = (
  ...
  `${ANIME_URL}?page[limit]=${pageLimit}&page[offset]=${offset}${getFilterString(
    filters
  )}`;

export const mangaUrl = (
  pageLimit: number = 10,
  offset: number = 0,
  filters?: Filters
) =>
  `${MANGA_URL}?page[limit]=${pageLimit}&page[offset]=${offset}${getFilterString(
    filters
  )}`;
